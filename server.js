...
setInterval(() => {
  ...
  http.get(`http://titin-wl.glitch.me/`);
...
client.on("guildMemberAdd", async member => {
  ...
  jimp.read(url, (err, ava) => {
    ...
    ava.getBuffer(jimp.MIME_PNG, (err, buf) => {
      ...
      c.send("**اهلا وسهلا بيك في سيرفر CLAN M5 دقيقه من وقتك اقرأه القوانين<#665994612518813710>**" )

      c.send(" **  اسم العضو **" + `${member}` + "");
...
client.on("guildMemberAdd", member => {
  ...
  member.guild.fetchInvites().then(guildInvites => {
    ...
    setTimeout(() => {
      ...
      logChannel.send(`** تمت دعوته بواسطه <@${inviter.id}>**
